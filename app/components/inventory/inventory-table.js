...
export default class InventoryInventoryTableComponent extends Component {
  @tracked inventory;
  @tracked ModalTableHeaders = [{ type: null, description: null, status: 'inactive' }];
  ...
  submitForm(event) {
    ...
    if (this.CreateInventoryEditItem != undefined) {
      console.log(
        'create inventory item to be edited',
  ...
  constructor() {
    ...
    try {
      ...
      this.ModalTableValue = inventoryType;
  ...
  async HandleInventoryType(title) {
    ...
    setTimeout(async () => {
      ...
      try {
        const inventoryType = await this.store.findAll(
    ...
    // setting input values to null when modal is open first
  ...
  handleAddInventory() {
    ...
    console.log(this.inputValues);
    // this.inputValues = {};
    const allNull = Object.values(this.inputValues).every(
      (value) => value === null,
    );
    console.log(allNull);
    ...
    if (!isMissingProperty) {
      ...
      if (this.inputvalueobj.length > 0) {
        ...
        this.inputvalueobj.forEach((singleObj) => {
          ...
          // function e padhiye dibo multiple value diye sekhane theke post hobe ***
        });
        // multiple post ***
  ...
  @action
  handleEdit(item) {
    // console.log('objecr', item);
    set(item, 'isClicked', true);
    // this.store
  }

  @action handleCancel(item) {
    set(item, 'isClicked', false);
  }
  ...
  async HandleCatagorytype(title) {
    ...
    setTimeout(async () => {
      ...
      // get api fetch
      try {
        const categoryTypes = await this.store.findAll(
          'inventory/inventory-category-type',
        );
        ...
        let inventoryCatagoryTypeObjects = this.inventoryCatagorytypedata.map(
          (element) => {
            return {
              id: element.id,
  ...
  HandleCatagory(title) {
    ...
    setTimeout(async () => {
      ...
      // fetch get api
      try {
        const inventoryCatagory = await this.store.findAll(
          'inventory/inventory-catagory',
        );
        // set it to the tracted property.
        ...
        let inventoryCatagoryTypeObjects = this.inventoryCatagory.map(
          (element) => {
            return {
              id: element.id,
  ...
  async HandleStoragetype(title) {
    ...
    setTimeout(async () => {
      ...
      try {
        ...
        this.inventoryStorageTypes = storgeType;
        // get and set value from the server,  obj that contains all the data that i need to show in the UI.(found from the server)
        let inventoryCatagoryTypeObjects = this.inventoryStorageTypes.map(
          (element) => {
            return {
              id: element.id,
              type: element.storage_type,
  ...
  async HandleAlocationType(title) {
    ...
    setTimeout(async () => {
      ...
      try {
        // Set isLoading to false when the data fetching is complete
        const AlocationType = await this.store.findAll(
          'inventory/allocation-type',
        );
        // set it to the tracted property.
        this.inventoryAlocationTypes = AlocationType;
        // get and set value from the server,  obj that contains all the data that i need to show in the UI.
        let inventoryCatagoryTypeObjects = this.inventoryAlocationTypes.map(
          (element) => {
            return {
              id: element.id,
  ...
  async HandleUnitMeasure(title) {
    ...
    setTimeout(async () => {
      ...
      // fetch get api
      try {
        const UnitMeasure = await this.store.findAll(
          'inventory/unit-of-measure',
        );
        // set it to the tracted property.
  ...
  @action
  async handleInputValueSave(item) {
    console.log(this.ModalTableValue);
    ...
    // status r jonno thik korte hobe ***
    // if (this.inputValues.hasOwnProperty('type') && this.inputValues.hasOwnProperty('description') && this.inputValues.hasOwnProperty('status')) {
    //   console.log('all', this.inputValues);

    //   let flag;
    //   if (this.inputValues.status == 'true') {
    //     flag = true
    //   }
    //   else {
    //     flag = false
    //   }
    //   item.status = flag;
    //   item.description = this.inputValues.description;
    //   item.type = this.inputValues.type;
    //   const inputvalue = this.inputValues;
    //   this.ModalTableValue = this.ModalTableValue;

    //   try {
    //     // post method
    //     this.store.findRecord('inventory/inventory-type', item.id).then(function (inventoryType) {
    //       // Modify attributes as needed
    //       inventoryType.set('inv_type', inputvalue.type);
    //       inventoryType.set('description', inputvalue.description);
    //       inventoryType.set('status', inputvalue.status);

    //       // Save changes
    //       inventoryType.save().then(function (savedInventoryType) {
    //         // Handle successful save
    //         console.log(savedInventoryType);
    //       })
    //     });
    //   } catch (error) {
    //     console.log(error);
    ...
    // } else if (this.inputValues.hasOwnProperty('description') && this.inputValues.hasOwnProperty('type')) {
    //   console.log('des and type');
    //   // change it to the store
    //   item.description = this.inputValues.description;
    //   item.type = this.inputValues.type;
    //   const inputvalue = this.inputValues;
    //   this.ModalTableValue = this.ModalTableValue;
    ...
    // } else if (this.inputValues.hasOwnProperty('description') && this.inputValues.hasOwnProperty('status')) {
    //   console.log('des n status', this.inputValues);
    ...
    //     this.store.findRecord('inventory/inventory-type', item.id).then(function (inventoryType) {
    //       inventoryType.set('description', inputvalue.description);
    //       // status boolean hote hobe
    //       inventoryType.set('active_status', inputvalue.status);
    ...
    // } else if (this.inputValues.hasOwnProperty('type') && this.inputValues.hasOwnProperty('status')) {
    //   console.log('type n status', this.inputValues);
    //   let flag;
    //   if (this.inputValues.status == 'true') {
    //     flag = true
    //   }
    ...
    // } else if (this.inputValues.hasOwnProperty('description')) {
    //   console.log('des', this.inputValues);
    //   item.description = this.inputValues.description;
    //   const inputvalue = this.inputValues;
    //   this.ModalTableValue = this.ModalTableValue;
    ...
    // } else if (this.inputValues.hasOwnProperty('type')) {
    //   console.log('type', this.inputValues);
    //   item.type = this.inputValues.type;
    //   const inputvalue = this.inputValues;
    //   this.ModalTableValue = this.ModalTableValue;
    ...
    // } else if (this.inputValues.hasOwnProperty('status')) {
    //   let flag;
    //   if (this.inputValues.status == 'true') {
    //     flag = true
    //   }
    ...
    // then after save click make isClicked false
    this.inputValues = {};
    set(item, 'isClicked', false);
  }
}
