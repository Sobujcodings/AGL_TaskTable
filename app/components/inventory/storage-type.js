...
export default class InventoryStorageTypeComponent extends Component {
    @service store;
    ...
    @action handleSave(item) {
        ...
        if (item.storage_type == '' || item.description == '') {
    ...
    constructor() {
        // try catch!
        super(...arguments);
        const inventoryType = this.store.findAll('inventory/storage-type');
    ...
    async HandleSave() {
        ...
        if (this.inputFieldNumbers.length == 0 || this.inputFieldNumbers == undefined || Object.values(lastObject).some(value => value === null || value === '')) {
            ...
            const newInventoryType = await this.store.createRecord('inventory/storage-type',
                {
                    storage_types: this.inputFieldNumbers.map((singleObj) => ({
                        storage_type: singleObj.type,
