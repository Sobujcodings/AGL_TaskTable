...
export default class InventoryUnitOfMeasureComponent extends Component {
  @service store;
  ...
  @action handleSave(item) {
    ...
    if (item.measure_type == '' || item.short == '') {
  ...
  constructor() {
    ...
    const inventoryType = this.store.findAll('inventory/unit-of-measure');
  ...
  handleAddInventory() {
    ...
    if (this.inputFieldNumbers.length == 0 || !Object.values(lastObject).some(value => value === null)) {
      this.inputFieldNumbers.pushObject({ 'type': null, 'short': null });
    } else {
  ...
  async HandleSave() {
    ...
    if (this.inputFieldNumbers.length === 1) {
      if (this.inputFieldNumbers[0].type === null && this.inputFieldNumbers[0].short === null || this.inputFieldNumbers[0].type !== null && this.inputFieldNumbers[0].short === null || this.inputFieldNumbers[0].type === null && this.inputFieldNumbers[0].short !== null) {
        alert("Please insert data");
        return
      }
    } else {
      if (lastObj.type === null && lastObj.short !== null || lastObj.type !== null && lastObj.short === null) {
        alert("Please insert data");
        return
      }
    }
    if (lastObj.type === null && lastObj.short === null) {
    ...
    const newInventoryType = await this.store.createRecord('inventory/unit-of-measure',
      {
        unit_of_measure: this.inputFieldNumbers.map((singleObj) => ({
          measure_type: singleObj.type,
          short: singleObj.short,
        }))
      },
    );
