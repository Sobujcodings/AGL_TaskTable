...
export default class InventoryAllocationTypeComponent extends Component {
    @service store;
    ...
    @action handleSave(item) {
        ...
        // validation
        if (item.alloc_type == '' || item.description == '') {
    ...
    constructor() {
        ...
        const inventoryType = this.store.findAll('inventory/allocation-type');
    ...
    handleAddInventory() {
        ...
        if (this.inputFieldNumbers.length == 0 || !Object.values(lastObject).some(value => value === null || value === '')) {
            this.inputFieldNumbers.pushObject({ 'type': null, 'description': null });
        } else {
    ...
    async HandleSave() {
        ...
        if (this.inputFieldNumbers.length == 0 || this.inputFieldNumbers == undefined || Object.values(lastObject).some(value => value === null || value === '')) {
            ...
            //   this value should be diff!!
            const newInventoryType = await this.store.createRecord('inventory/allocation-type',
                {
                    allocation_types: this.inputFieldNumbers.map((singleObj) => ({
                        alloc_type: singleObj.type,
